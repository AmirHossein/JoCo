<?php
/*********************************
Snippet : JoCo (Jot Comments snippet) : JoCo class
Version : 0.9.1.4
...
class JoCo {
	...
	var $params;	// snippet parameters array
	var $userId;
	var $JoCoUser;

	/**
	 * JoCo snippet constructor
	 *
	 * @param	array		$params : Snippet parameters associative array.
	 * @return	void
	 */		
	function JoCo($params)
	{
		global $modx;
		
		$params['disabledForUser'] = $this->PermiParamCols($params['disabledForUser']);
		$params['enabledForUser'] = $this->PermiParamCols($params['enabledForUser']);
		$params['disabledForAll'] = $this->explodeByComma($params['disabledForAll']);
		$this->params = $params;
		...
		$this->gv = array(
			...
			,'sniPath' => 'assets/snippets/JoCo/'
			,'lang' => ( isset($this->params['lang']) ) ? $this->params['lang'] : $modx->config['manager_language']
			,'dir' => ( isset($this->params['dir']) ) ? $this->params['dir'] : ($this->params['lang']=="persian" ? "rtl" : "ltr")
			,'lang_attr' => $modx->config['manager_lang_attribute']
			,'alignRight' => ( isset($this->params['dir']) ) ? ($this->params['dir']=="rtl" ? "right" : "left") : "left"
			,'alignLeft' => ( isset($this->params['dir']) ) ? ($this->params['dir']=="rtl" ? "left" : "right") : "right"
			,'mKey' => $modx->documentIdentifier // page ID
			,'pageUrl' => $modx->makeUrl($modx->documentIdentifier) . ($modx->getDocumentMethod()=="alias" ? "?" : "&")
		);
		
		// include lang file. it contains $JoCo_lang
		if( !empty($this->params['lang']) )
		{
			include_once $this->gv['sniPath'].'langs/'.$this->params['lang'].'.php';
			$this->gv['lang'] = $this->params['lang'];
		}
		else
		{
			if( file_exists($this->gv['sniPath'].'langs/'.$this->gv['lang'].'.php') )
				include_once $this->gv['sniPath'].'langs/'.$this->gv['lang'].'.php';
			else
				include_once $this->gv['sniPath'].'langs/english.php';
		}
		$this->lang = $JoCo_lang;
		
		// includeneedPermissions.php file. it contains elementsPermission class  and CrElements and generals methods.
		include($this->gv['sniPath'].'includes/needpermissions.php');
		...
		include($this->gv['sniPath'].'includes/customactions.php');
		$this->customFunctions = new customFunctions();	
		
		if(isset($_SESSION['mgrInternalKey']))
		{
			$this->userId = $_SESSION['mgrInternalKey'];
			$this->JoCoUser = $_SESSION['mgrInternalKey'];
		}
		if(isset($_SESSION['webInternalKey']))
		{
			$this->userId = $_SESSION['webInternalKey'];
			$this->JoCoUser = $_SESSION['webInternalKey'] * -1;
		}
	...
	function changeComment($ids, $update, $type)
	{
		...
		foreach($comment_ids as $comment_id)
		{
			...
			$this->managerLog( str_replace("[+comment_id+]", $comment_id, $this->lang[$type.'_log']), $comment_id );
		}
	}
	...
	function getIndv($r, $tpl, $checkedElements, $count=false, $edit=false)
	{
		...
		*/
		...
		$myComment = ($this->JoCoUser == $r['createdby']) ? 'myComment' : '';
		...
		$tpl = $this->replace_all($this->gv, $tpl);	// replace geeral vars
	...
	function hasPermission()
	{
		...
		if( isset($_SESSION['mgrValidated']) )
		{
			...
			if($modx->db->getRecordCount($select) == 0)
			{
				...
				{
					$insert = array(
						"internalKey"=>$_SESSION['mgrInternalKey'], "publish"=>1
						,"unpublish"=>1, "edit"=>1, "remove"=>1, "submit"=>1, "webUsers"=>1
						,"ip"=>1, "logging"=>1, "viewAll"=>1, "viewPublished"=>1
						,"viewUnpublished"=>1, "publishedDocs"=>1234, "createdDocs"=>1234
						,"editedDocs"=>1234, "search"=>1, "permission"=>1, "summary"=>1
					);
				}
				else
				{
					$insert = array("internalKey" => $_SESSION['webInternalKey']);
				}
		...
		{
			$select = $modx->db->select("*", $this->gv['joco_permissions_tbl'], "internalKey=".($_SESSION['webInternalKey'] * -1));
			
			// this user is new so JoCo inserts a new row for him/her
			if($modx->db->getRecordCount($select) == 0)
			{
				$insert = array("internalKey" => $_SESSION['webInternalKey'] * -1);
		...
		$newPermi = $this->params;
		foreach($Row as $permis => $val)
			if( $newPermi[$permis] == null )
				$newPermi[$permis] = $Row[$permis];
		
		$this->permission = $newPermi;
		
		// Get user last active
		$this->permission['lastActive'] = $this->id_to_fullname($this->JoCoUser, "lastlogin", "CHECKIT");
	...
	{
		if( str_str($WHERE, "AND $piece") )
			$replace = "AND $piece";
		elseif( str_str($WHERE, "$piece AND") )
			$replace = "$piece AND";
		else
			$replace = $piece;
	 ...
	 *
	 * @param	string		$str : input string that have $deli
	 * @param	string		$deli : delimeter, default: comma
	 * @return	array		array contains exploded strings
	 */	

	function explodeByComma($str, $deli=",")
	{
		$str = str_replace(" ", "", $str);	//remove spaces
		if( strpos($str, $deli) )
		{
			// remove bad comma ing
			$invalid_comma = array("$deli ", " $deli", " $deli ");
			$str = str_replace($invalid_comma, $deli, $str);
	 ...
	 */	
	function id_to_fullname($id, $field="fullname", $table="user_attributes", $where=false)
	{
		global $modx;
		...
		$where = (!$where) ? "internalKey=".abs($id) : "internalKey=".abs($id)." AND $where";
	...
	function fileContent($url)
	{
		if( function_exists("file_get_contents") )
			return file_get_contents($this->gv['sniPath'].$url);
		else
		{
			$output = "";
			$fp = fopen($this->gv['sniPath'].$url , "r");
			while( ! feof($fp) )
				$output .= fgets($fp);
			return $output;
		}
	...
	/**
	 * gets an associative array and filters the values by escape and stripTags functions.
	 *
	...
	function escape($assoc, $validTags="<br>")
	{
		...
		{
			if( is_string($val) )
				$val = $modx->stripTags( $modx->db->escape($val), $validTags );
			$output[$key] = $val;
		}
		return $output;
	}
	 ...
	 * @return	void
	 */	
	function managerLog($message, $commentId)
	{
		...
		if( $this->JoCoUser < 0)
		{
			$row = $modx->db->getRow( $modx->db->select("uparent", $this->gv['jot_content_tbl'], "id=$commentId") );
			
			$logs = '
		<tr>
			<td>'. $this->id_to_fullname($this->userId, "fullname", "web_user_attributes"). ' <small>('. $this->userId .')</small></td>
			<td>'. $message .'</td>
			<td>'. $this->JoCoDate() .'</td>
			<td>'. $commentId .'</td>
			<td><a href="'. $modx->makeUrl($row['uparent']) .'" target="blank">'. $this->id_to_title($row['uparent']) .'</a></td>
		</tr>
		<!--[+newRow+]-->';

			$logContent = str_replace('<!--[+newRow+]-->', $logs , $this->fileContent('logs/logs.html') );
			$fp = fopen($this->gv['sniPath'].'logs/logs.html', "w");
	...
	function jdate($stamp, $format="d F Y H:i")
	{
		...
		if( file_exists($this->gv['sniPath']."includes/jcalender.class.php") )
		{
			require_once($this->gv['sniPath']."includes/jcalender.class.php");
			$cal = new Calendar();
		}
		else
	...
	function setPaginate($all, $perPage, $where, $curr)
	{
		...
		for($i=1 ; $i<=$allPages ; $i++)
		{
			if($curr == $i) // this is current page
				...
				$nums[] = '<span class="pageNum"><a href="'.$this->gv['pageUrl'].'page='.$i.$where.'">'.$i.'</a></span>'; // an inline class!!!
		}
		return $nums;
	}
	...
	function myZone($uparent)
	{
		...
		if( $docInfo["createdby"] == $this->userId )
		{
		...
		if( $docInfo["editedby"] == $this->userId )
		{
		...
		if( $docInfo["publishedby"] == $this->userId )
		{
	...
	/**
	 * Parse &disabled parameter
	 *
	 * @param	string	$params_disabled : parameter value.
	 * @return	array	associative array, keys are columns of "joco_permissions" table and value for each key is an array containing webuser IDs
	 */
	function PermiParamCols($params_disabled)
	{
		// &disabled=`logging:1,2,3|search:1,2,3`
		$params_disabled = str_replace(" ", "", $params_disabled);	// remove spaces
		$disabledParts = explode("|", $params_disabled);
		
		if($params_disabled == "")
			$disabledParts = array();
		
		$disabledArray = array();
		foreach($disabledParts as $disabledPart)
		{
			list($key, $value) = explode(":", $disabledPart);
			$disabledArray[$key] = explode(",", $value);
		}
		...
		foreach($cols as $col)
			if(empty($disabledArray[$col]))
				$disabledArray[$col] = array();
	...
	function checkReferer()
	{
		...
		else
		{
			...
			if( isset($_GET['WHERE']) || isset($GET['show']) )
			{
				...
				if( ! (isset($keys['id']) && $keys['id']==$this->gv['mKey']) )
					return false;
			}
		}
		return true;
