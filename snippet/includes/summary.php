...
File map:
	...
	# Include HTML tpl file : 49
	# Get number of comments for each document : 52
	# Get number of subscriptions for each document : 91
	# Set sort link : 120
	# Set output : 157
		# Sorting by commnets : 158
		# Main data loop : 172
		# Do pagination : 174
		# Set paginate : 225
		# Complete output : 241
...
$SUMMARY = $JoCo->fileContent('tpls/summary.html');
...
if( array_key_exists('sortDir', $_GET) )
{
	if($_GET['sortDir'] == "ASC")
		$sortLink = '
			<a href="[+pageUrl+]sortBy=comments&sortDir=DESC'.$where.'" title="'.$JoCo->lang['DESC'].'">
				<img src="'.$JoCo->gv['sniPath'].'/images/up_arrow.png" width="16" alt="'.$JoCo->lang['DESC'].'" />
			</a>
		';
	if($_GET['sortDir'] == "DESC")
		$sortLink = '
			<a href="[+pageUrl+]sortBy=comments&sortDir=ASC'.$where.'" title="'.$JoCo->lang['ASC'].'">
				<img src="'.$JoCo->gv['sniPath'].'/images/down_arrow.png" width="16" alt="'.$JoCo->lang['ASC'].'" />
			</a>
		';
}
else
...
foreach($comment['comments'] as $uparent => $count)	// Main data loop
{
	...
	$table .= '
		<tr>
			...
			<td align="center">
				<a href="[+pageUrl+]WHERE=uparent='.$uparent.'&reffer=1" title="'.$JoCo->lang['see_this_comments'].'">
						<img src="'.$JoCo->gv['sniPath'].'/images/home.png" width="20" alt="'.$JoCo->lang['see_this_comments'].'" />
					</a>
			</td>
			...
			<td align="center" id="subscriptions_'.$id.'">
				'.(
					...
					'<a href="javascript:getSubscriptions('.$uparent.','.$id.');" title="'.$JoCo->lang['get_subscriptions'].'">
						<img src="'.$JoCo->gv['sniPath'].'/images/search_magnifier.png" width="16" alt="'.$JoCo->lang['get_subscriptions'].'" />
					</a>'
					:
					'<img src="'.$JoCo->gv['sniPath'].'/images/search_magnifier_dis.png" width="16" title="'.$JoCo->lang['there_is_no_subscription_here'].'" alt="'.$JoCo->lang['there_is_no_subscription_here'].'" />'
				).'
			</td>
...
$nextPage = '<a href="[+pageUrl+]page='.($page+1).$where.'">'.$JoCo->lang['next_page'].'</a>';
$prePage = '<a href="[+pageUrl+]page='.( $page==1 ? 1 : ($page-1)).$where.'">'.$JoCo->lang['previous_page'].'</a>';
