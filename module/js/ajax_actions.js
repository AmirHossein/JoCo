...
function ajaxAction(action, id)
{
	...
	xmlHttp.open("GET" , "index.php?a=112&id=" +mKey+ "&comment_action=" + action + "&comment_id=" + id + query, true);
	
	xmlHttp.onreadystatechange=function() 
	{
		if(xmlHttp.readyState==1)
		{
			document.getElementById("loading_" + id).innerHTML  = "<p><img src=\"" +baseUrl+modulePath+ "images/loading.gif\" />" +lang_loading+ "</p>";
		...
		if(xmlHttp.readyState==4 && xmlHttp.status==200)
		{
			...
			// disable publish checkbox TD
				document.getElementById("pub_col_"+id).style.backgroundImage="url('"+baseUrl+modulePath+"images/publish-col_dis.png')";
			//disable checkbox	
				document.getElementById("pub_chkbx_" + id).setAttribute("disabled","disabled");
			//enable upublish checkbox TD
				document.getElementById("unpub_col_"+id).style.backgroundImage="url('"+baseUrl+modulePath+"images/unpublish-col.gif')";
			...
			// disable publish action image	
				document.getElementById("img_act_publish_" + id).setAttribute("src", baseUrl+modulePath + "images/publish_dis.png");
			...
			// enable unpublish action image
				document.getElementById("img_act_unpublish_" + id).setAttribute("src", baseUrl+modulePath + "images/unpublish.png");
			// set new status bg
			...
			if(action == "unpublish")
			{
				...
				document.getElementById("unpub_col_"+id).style.backgroundImage="url('"+baseUrl+modulePath+"images/unpublish-col_dis.png')";
				document.getElementById("unpub_chkbx_" + id).setAttribute("disabled","disabled");
				document.getElementById("pub_col_"+id).style.backgroundImage="url('"+baseUrl+modulePath+"images/publish-col.gif')";
				...
				document.getElementById("img_act_unpublish_" + id).setAttribute("src", baseUrl+modulePath + "images/unpublish_dis.png");			
				// enable Publish action
				document.getElementById("act_publish_" + id).setAttribute("href","javascript:ajaxAction('publish','"+id+"');");
				document.getElementById("img_act_publish_" + id).setAttribute("src", baseUrl+modulePath + "images/publish.png");
