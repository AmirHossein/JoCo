...
function getPermissions(id)
{
	...
	xmlHttp.open("GET" , "index.php?a=112&id=" +mKey+ "&permission=1&getPermissions=" + id, true);
...
function moveUser(dir, pos)
{
	...
	if( typeof id == "undefined" || id == "" )	return false;
	var xmlHttp = ajaxRequest();
	xmlHttp.open("GET" , "index.php?a=112&id=" +mKey+ "&permission=1&dir=" + dir + "&moveUser=" + id, true);
	xmlHttp.onreadystatechange=function() 
	{document.getElementById(pos + "existUsers").innerHTML  = xmlHttp.responseText;
		if(xmlHttp.readyState==1)
			document.getElementById("moveButtons").innerHTML  = '<p><img src="' +baseUrl+modulePath+ 'images/loading.gif" />' +lang_loading+ '</p>';
...
{
	var xmlHttp = ajaxRequest();
	xmlHttp.open("GET" , "index.php?a=112&id=" +mKey+ "&permission=1&refreshList=" +pos, true);
	xmlHttp.onreadystatechange=function() 
	{
		if(xmlHttp.readyState==1)
			document.getElementById(pos + "existUsers").innerHTML  = '<p><img src="' +baseUrl+modulePath+ 'images/loading.gif" />' +lang_loading+ '</p>';
...
function changeEl(src, pos)
{
	...
	{
		document.getElementById("moveIn").innerHTML = '<img src="'+baseUrl+modulePath+'images/right_arrow_dis.png" alt="'+lang_add_user+'" />';
		document.getElementById("moveOut").innerHTML = '<a href="javascript:moveUser(\'out\',\''+pos+'\');"  title="'+lang_remove_user+'"><img src="'+baseUrl+modulePath+'images/left_arrow.png" alt="'+lang_remove_user+'" /><a/>';
		...
		document.getElementById("getPermi").innerHTML = '<a href="javascript:getPermissions(\''+res+'\');" title="'+lang_get_permissions+'"><img src="'+baseUrl+modulePath+'images/process.png" alt="'+lang_get_permissions+'" /></a>';
	}
	
	if(src == "all") // the exists user has been selected. it disables "moveOut" & "getPermissions" and enables "moveIn".
	{
		document.getElementById("moveIn").innerHTML = '<a href="javascript:moveUser(\'in\',\''+pos+'\');" title="'+lang_add_user+'"><img src="'+baseUrl+modulePath+'images/right_arrow.png" alt="'+lang_add_user+'" /><a/>';
		document.getElementById("moveOut").innerHTML = '<img src="'+baseUrl+modulePath+'images/left_arrow_dis.png" alt="'+lang_remove_user+'" />';
		document.getElementById("getPermi").innerHTML = '<img src="'+baseUrl+modulePath+'images/process_dis.png" alt="'+lang_get_permissions+'" />';
	}
}